...
private:
    void GNSSCB(const sensor_msgs::NavSatFixConstPtr &msg) {
        // std::cout << "gps status: " << static_cast<int>(msg->status.status) << std::endl;
        ...
        // after initialization, /init_odom will not publish again
        if (!initENU) {
            ...
            init_msg.pose.pose.position.z = lla[0];
            init_msg.pose.pose.position.x = lla[1];
            init_msg.pose.pose.position.y = lla[2];
        ...
        odom_msg.pose.pose.position.z = enu(0);
        odom_msg.pose.pose.position.x = enu(1);
        odom_msg.pose.pose.position.y = enu(2);
        ...
        pose.pose.position.z = enu(0);
        pose.pose.position.x = enu(1);
        pose.pose.position.y = enu(2);
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "legoloam_GNSS");
    ros::NodeHandle nh;
    GNSSOdom gps(nh);
